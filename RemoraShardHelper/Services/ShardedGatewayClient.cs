    ...
    public async Task<Result> RunAsync(CancellationToken ct = default)
    {
        ...
        if (maxStartup is 1)
        {
            for (var shardIndex = 0; shardIndex < clients.Length; shardIndex++)
            {
                ...
                while (GetClientConnectionStatus(client) is not GatewayConnectionStatus.Connected || res.IsCompleted)
                {
        ...
        else
        {
            ...
            for (var burstIndex = 0; burstIndex < bursts.Length; burstIndex++)
            {
                ...
                while (burst.Any(c => GetClientConnectionStatus(c) is not GatewayConnectionStatus.Connected))
                {
